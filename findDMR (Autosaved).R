...
k <- 1000
count <- 1
out <- data.frame(region=NA, chr=NA, start=NA, stop=NA, num_cpg=NA, area=NA)

for (i in 1:dim(temp)[1]) {
    #starts a new region here
    new <- data.frame(region=NA, chr=NA, start=NA, stop=NA, num_cpg=NA, area=NA)
