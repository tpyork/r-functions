# Clunky fix to estimate cell counts for cord blood samples
# need to adjust the referenceRGset within the minfi namespace
# this code brings along other functions that exist in minfi namespace
...
estimateCellCounts.cord <- function (rgSet, compositeCellType = "CordBlood", processMethod = "auto", 
          probeSelect = "auto", cellTypes = c("CD8T", "CD4T", "NK", 
                                              "Bcell", "Mono", "Gran"), referencePlatform = c("IlluminaHumanMethylation450k", 
          ...
          returnAll = FALSE, meanPlot = FALSE, verbose = TRUE, ...) 
{
  ...
  referenceRGset$X <- as.character(referenceRGset$X)
  referenceRGset$Plate_ID <- as.character(referenceRGset$Plate_ID)
  referenceRGset$Sample.Well <- as.character(referenceRGset$Sample.Well)
  referenceRGset$Hyb.date <- as.character(referenceRGset$Hyb.date)
  referenceRGset$Image.date <- as.character(referenceRGset$Image.date)
  referenceRGset$Sex <- as.character(referenceRGset$Sex)
  referenceRGset$Age <- as.character(referenceRGset$Age)
  referenceRGset$Individual.ID <- as.character(referenceRGset$Individual.ID)
  referenceRGset$Array <- as.character(referenceRGset$Array)
  referenceRGset$Slide <- as.character(referenceRGset$Slide)
  referenceRGset$Basename <- as.character(referenceRGset$Basename)
  referenceRGset$predictedSex <- as.character(referenceRGset$predictedSex)
